...
$(document).ready(function() { // вся магия после загрузки страницы
	$('a#go2').click(function(event) { // ловим клик по ссылки с id="go"
		...
		$('#overlay').fadeIn(400, // сначала плавно показываем темную подложку
		function() { // после выполнения предъидущей анимации
			$('#modal_form2').css('display', 'block') // убираем у модального
														// окна display: none;
			.animate({
				opacity : 1,
				top : '50%'
	...
	/* Закрытие модального окна, тут делаем то же самое но в обратном порядке */
	$('#modal_close2, #overlay').click(function() { // ловим клик по крестику
													// или подложке
		$('#modal_form2').animate({
			opacity : 0,
			top : '45%'
...
$(document).ready(function() { // вся магия после загрузки страницы
	$('a#go3').click(function(event) { // ловим клик по ссылки с id="go"
		...
		$('#overlay').fadeIn(400, // сначала плавно показываем темную подложку
		function() { // после выполнения предъидущей анимации
			$('#modal_form3').css('display', 'block') // убираем у модального
														// окна display: none;
			.animate({
				opacity : 1,
				top : '50%'
	...
	/* Закрытие модального окна, тут делаем то же самое но в обратном порядке */
	$('#modal_close3, #overlay').click(function() { // ловим клик по крестику
													// или подложке
		$('#modal_form3').animate({
			opacity : 0,
			top : '45%'
